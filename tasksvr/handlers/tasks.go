...
import (
	...
	"github.com/alexsirr/info344-in-class/tasksvr/models/tasks"
)
...
func (ctx *Context) TasksHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		...
		tasks, err := ctx.taskStore.GetAll(false)
		if err != nil {
	...
	case "POST":
		...
		task, err := ctx.taskStore.Insert(nt)
		if err != nil {
...
func (ctx *Context) SpecificTaskHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "PATCH":
		...
		task, err := ctx.taskStore.Update(oid, tu)
		if err != nil {
